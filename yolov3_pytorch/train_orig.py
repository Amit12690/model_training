...
if __name__ == "__main__":
    ...
    for epoch in range(opt.epochs):
        ...
        if AP.mean() >= best_mAP:
            ...
            if opt.clear_old_ckpt.lower() == 'true':
                delete_old_ckpts(checkpoint_folder,extension='pth')
            torch.save(model.state_dict(), f"%s/yolov3_ckpt_bestMAP_%f.pth"%(checkpoint_folder,best_mAP))
            
        if epoch % opt.checkpoint_interval == 0:
            torch.save(model.state_dict(), f"%s/yolov3_ckpt_%d.pth" %(checkpoint_folder,epoch))
